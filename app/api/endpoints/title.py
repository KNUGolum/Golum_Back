...
async def getTitleShopList(
    db: Session = Depends(getDb),
    currentUser: User = Depends(getCurrentUser)
):
    shopTitles = title.getShopTitles(db)
    ownedTitleIds = title.getOwnedTitleIds(db, currentUser.id)

    return TitleShopListResponse(
        message="상점 목록 조회가 완료되었습니다.",
        titles=[
            ShopTitle(
                titleId=shopTitle.id,
                name=shopTitle.name,
                grade=shopTitle.grade,
                price=shopTitle.price,
                isOwned=shopTitle.id in ownedTitleIds
            )
            for shopTitle in shopTitles
        ]
    )
